...
export default function OrbitingTags() {
  ...
  useAnimationFrame((t, delta) => {
    ...
    setRotation((prev) => ({
      x: prev.x + (moveY * 0.002 + 0.001) * (delta / 10),
      y: prev.y + (moveX * 0.002 - 0.002) * (delta / 10),
    }));
  });
...
function Tag({
  ...
  text: string;
  index: number;
  initialPos: { phi: number; theta: number };
  ...
  const rotatedX2 =
    rotatedX1 * Math.cos(rotation.y) + rotatedZ1 * Math.sin(rotation.y);
  const rotatedY2 = rotatedY1;
  const rotatedZ2 =
    -rotatedX1 * Math.sin(rotation.y) + rotatedZ1 * Math.cos(rotation.y);

  const finalX = rotatedX2;
  const finalY = rotatedY2;
  const finalZ = rotatedZ2;

  const scale = (finalZ + 2 * RADIUS) / (3 * RADIUS);
  const opacity = Math.max(0.1, (finalZ + RADIUS) / (2 * RADIUS));
